...
plot.expression.with.dots <- function(cds.subset,grouping) {
  ...
  dplot <- ggplot(data = expr.df10,mapping = aes_string(x = grouping,y = "expression")) +
    facet_wrap(facets = .~gene,scales = "free") +
    geom_point(position = position_jitter(),size = 0.5,alpha=0.7) +
